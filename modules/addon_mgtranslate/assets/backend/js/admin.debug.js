			...
			zgfm_variable.externalVars = {};
				...
				$( "textarea" ).each( function( index, element ){
					...
					if(parseInt(tmpval.length)===0){
						let newVal=tmpObj.parent().prev().html();
						tmpObj.val(newVal);
								...
								if(parseInt(inputNewLang.length)===0){
									...
									success: function(result){
										...
										$('#uifm_modal_msg .sfdc-modal-footer').html(result.data.html_footer);
											...
											var re_url = uiform_vars.url_admin + 'admin.php?page=zigaform-translation';
				...
				$('#zgfm-page-translation textarea').on('focus', function() {
					...
					data: {
						...
						'side':$('#mgtr-input-side').val()
						 ...
						 beforeSend: function () {
							let tmpmsg=`<img src="${uiform_vars.url_plugin}/assets/backend/image/ajax-loader-black.gif">`;
